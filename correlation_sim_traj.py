...
# compute propensity
def cal_propensity(ini,*args): # arguments: x_1, x_2, \gamma, h
...
def gillespie_draw(c_cal_propensity,index,*args): # arguments: x_1, x_2, \gamma, h
...
def gillespie_ssa(c_cal_propensity,timepoints,initial_index,*args): # arguments: x_1, x_2, \gamma, h
    ...
    while i<size:
        ...
        for j in range(i_time,i):
            record[j,:] = [timepoints[j],index_now]
        i_time = i
    return record

def test_gillespie_ssa(c_cal_propensity,timepoints,initial_index,*args): # arguments: x_1, x_2
    m_h = 7
    m_dist_sto = np.zeros((21,8))
    m_dist_num = np.zeros((21,8))
    m_dist_ana = np.zeros((21,8))

    for m_g in tqdm.tqdm(range(0,21)):
        np.random.seed(m_g)
        m_traj = gillespie_ssa(c_cal_propensity,timepoints,initial_index,*args,m_g,m_h)
        t_cnt = 0
...
def cal_fluctuation(c_cal_propensity,timepoints,initial_index,*args): # arguments: x_1, x_2
    record = gillespie_ssa(c_cal_propensity,timepoints,initial_index,*args)
    len_record = np.shape(record)
    len_record = len_record[0]
    record_y = np.zeros(len_record)
    for i in range(len_record):
        record_y[i] = sts.m_y_label[int(record[i,1])] # for clear calculation, add all the labels by 1
    
    np.savetxt(f".\\resl_corr\\record_y.dat",record_y,fmt='%.2f')
    
    mean_state = np.mean(record_y) # average of y labels
    flc_state = np.std(record_y) # standard deviation of y labels
    return (mean_state,flc_state)
...
m_rcd_flc = np.zeros((16,2))
m_iterate = 0
for i in range(4):
    m_g = (i+1)*5
    for j in range(4):
        m_h = 2*j+6
        m_rcd_flc[m_iterate] = cal_fluctuation(cal_propensity,timepoints,initial_index,m_x1,m_x2,m_g,m_h)
        m_iterate = m_iterate+1
filename = f".\\resl_corr\\mean_fluc.dat"
np.savetxt(filename,m_rcd_flc,fmt='%.4e',header='g=5,10,15,20,h=6,8,10,12; loop g, then loop h')

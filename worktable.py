...
m_accu_num = np.zeros((16,8))
for i in range(16):
    m_g = i
    m_mat_k = sts.cal_mat_k(m_x1,m_x2,m_g,m_h)
    m_ness = sts.cal_ness_distribution(m_mat_k)
    for k in range(8):
        m_accu_num[i,k] = m_ness[k]

filename = f"..\\RESULTS\\01062023_testanasim\\ana.dat"
with open(filename,'r') as f:
    m_accu_ana = np.loadtxt(f)

plt.figure()
for i in range(8):
    plt.plot(range(16),m_accu_num[:,i],color=m_colorbar[i],label='P(%d,%d,%d)'%(sts.m_m_v[i],sts.m_a_v[i],sts.m_y_v[i]))
    plt.plot(range(16),m_accu_ana[:,i],marker='o',linewidth=0,color=m_colorbar[i])
plt.legend()
plt.xlabel('\u03B3')
plt.ylabel('accuracy')
plt.show()